 ...
 *
 * UX rules:
 *  • When the user has no group, Arena shows only two actions:
 *      1) Вступить в группу
 *      2) Создать группу
 *  • Joining is done by an invitation LINK, never by exposing a code.
 *  • A Telegram startapp invitation link automatically joins the user.
 *  • Immediately after create/join the user is asked for S/B/D results.
 *  • Arena data is independent from Progress/workouts.
 *  • The server remains the source of truth for one-group membership.
 * ============================================================= */
(function () {
  'use strict';
  ...
  var CSS = [
    ...
    '.thar-name{flex:1;font-size:12px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
  ...
  function busy(key,on) { st.busy[key]=on; try { document.querySelectorAll('.thar-btn[data-busy="'+key+'"],.thar-action[data-busy="'+key+'"]').forEach(function(x){x.disabled=on;}); } catch(e){} }
  ...
  function networkFail(r,retry) {
    var msg = r && r.status === 409 ? 'Ты уже состоишь в другой группе.' : r && r.reason === 'telegram-required' ? 'Открой Train Hard внутри Telegram.' : r && (r.reason === 'network' || r.status === 0) ? 'Нет соединения с сервером. Проверь интернет и попробуй снова.' : 'Сервер недоступен (' + ((r&&r.status)||'нет ответа') + '). Попробуй позже.';
  ...
  function parseInviteLink(value) {
    ...
    try {
      var u = new URL(raw);
      var p = u.searchParams.get('startapp');
      if (p && /^[A-Za-z0-9_-]{8,64}$/.test(p)) return p;
    } catch (e) {}
    var m = raw.match(/[?&]startapp=([A-Za-z0-9_-]{8,64})/i);
    return m ? m[1] : null;
  }
  ...
  function openModal() {
    if (open) return;
    open = true; ensureDom();
    root = el('div','thar-root'); root.id='thar-root';
    panel = el('div','thar-panel');
    var head=el('div','thar-head'); var title=el('div','thar-title','ARENA'); title.id='thar-title';
    var x=el('button','thar-x','✕'); x.addEventListener('click',closeModal);
    head.appendChild(title); head.appendChild(x); panel.appendChild(head);
    var b=el('div','thar-body'); b.id='thar-body'; panel.appendChild(b); root.appendChild(panel);
    root.addEventListener('click',function(e){if(e.target===root)closeModal();});
    document.body.appendChild(root); document.body.classList.add('thch-open');
    goHome();
  }
  ...
  function renderHero(subtitle) {
    var h=el('section','thar-hero');
    h.appendChild(el('div','thar-kicker','TRAIN HARD · COMPETITION')); h.appendChild(el('div','thar-hero-title','ARENA'));
    h.appendChild(el('div','thar-hero-copy',subtitle || 'Группы, силовые результаты и общий рейтинг. Твои результаты в Arena не зависят от тренировочного журнала.'));
    return h;
  }

  function actionButton(icon,title,copy,primary,fn) {
    var b=el('button','thar-action'+(primary?' primary':'')); b.type='button';
    b.appendChild(el('div','thar-action-icon',icon)); var c=el('div'); c.appendChild(el('div','thar-action-title',title)); c.appendChild(el('div','thar-action-copy',copy)); b.appendChild(c); b.appendChild(el('div','thar-arrow','›'));
    b.addEventListener('click',function(){buzz(10);sound('tap');fn();}); return b;
  }

  async function goHome(autoCode) {
    st.view='home'; st.group=null; setTitle('ARENA'); var b=body(); clear(); b.appendChild(skeletons(2));
    var r=await API.request('GET','/api/groups'); if(!alive())return;
    if(!r.ok){clear();b.appendChild(networkFail(r,function(){goHome(autoCode);}));return;}
    var groups=(r.body&&r.body.groups)||[];
    if(groups.length){ openGroup(groups[0].group ? groups[0].group.id : groups[0].id,false); return; }
    ...
    var actions=el('div','thar-card');
    actions.appendChild(actionButton('↗','Вступить в группу','Открой приглашение или вставь ссылку от друга.',true,showJoin));
    actions.appendChild(actionButton('＋','Создать группу','Создай свою группу и пригласи участников ссылкой.',false,showCreate));
    b.appendChild(actions);
    if(autoCode) autoJoin(autoCode);
  }

  function showJoin(){
    var b=body(); clear(); setTitle('ВСТУПИТЬ'); b.appendChild(renderHero('Вставь ссылку-приглашение Train Hard. Код из ссылки скрыт — после открытия ссылка сама подключит тебя к группе.'));
    var card=el('div','thar-card'); var input=el('input','thar-input'); input.placeholder='https://t.me/trainhard_power_bot/trainhard?startapp=…'; input.autocomplete='off';
    var btn=el('button','thar-btn red wide','Вступить в группу'); btn.dataset.busy='join';
    btn.addEventListener('click',function(){var code=parseInviteLink(input.value); if(!code){input.style.borderColor='#ef233c'; return;} joinByCode(code,false);});
    card.appendChild(el('div','thar-sub','Ссылка-приглашение')); card.appendChild(input); card.appendChild(btn);
    var back=el('button','thar-btn ghost wide','← Назад'); back.style.marginTop='9px'; back.addEventListener('click',goHome); card.appendChild(back); b.appendChild(card);
  }

  async function showCreate(){
    var b=body(); clear(); setTitle('СОЗДАТЬ ГРУППУ'); b.appendChild(renderHero('После создания сразу введи свои присед, жим и становую — они станут основой рейтинга.'));
    var card=el('div','thar-card'); card.appendChild(el('div','thar-sub','Название группы'));
    var input=el('input','thar-input'); input.placeholder='Например: POWERLIFTING FRIENDS'; input.maxLength=80; input.autocomplete='off';
    var btn=el('button','thar-btn red wide','Создать группу'); btn.dataset.busy='create';
    btn.addEventListener('click',async function(){var name=input.value.trim(); if(name.length<3){input.style.borderColor='#ef233c';return;} busy('create',true); var r=await API.request('POST','/api/groups',{name:name}); busy('create',false); if(r.ok&&r.body&&r.body.group){buzz([12,40,12]);openGroup(r.body.group.id,true);return;} clear();body().appendChild(networkFail(r,showCreate));});
    card.appendChild(input); card.appendChild(btn); var back=el('button','thar-btn ghost wide','← Назад'); back.style.marginTop='9px'; back.addEventListener('click',goHome); card.appendChild(back); b.appendChild(card);
  }

  async function autoJoin(code){
    if(!code || wasInviteProcessed(code)) return;
    await joinByCode(code,true);
  }

  async function joinByCode(code,fromDeepLink){
    busy('join',true); var r=await API.request('POST','/api/invites/'+encodeURIComponent(code)+'/join',{}); busy('join',false);
    if(!alive())return;
    if(r.ok&&r.body&&r.body.group){rememberInvite(code);buzz([15,40,15]);openGroup(r.body.group.id,true);return;}
    if(fromDeepLink && r.status===409){goHome();return;}
    var b=body(); clear(); b.appendChild(networkFail(r,goHome));
  }

  function renderStatsCard(edit,onboarding){
    var card=el('div','thar-card'); card.appendChild(el('div','thar-sub',onboarding?'Твои силовые':'Мои результаты'));
    if(onboarding) card.appendChild(el('div','thar-note','Введи последние рабочие максимумы. Их можно изменить позже.'));
    var grid=el('div','thar-stat-grid');
    [['squat','Присед'],['bench','Жим'],['deadlift','Становая']].forEach(function(x){var wrap=el('div','thar-stat');wrap.appendChild(el('div','thar-stat-label',x[1]));if(edit){var inp=document.createElement('input');inp.type='number';inp.min='0';inp.max='1000';inp.step='0.5';inp.inputMode='decimal';inp.value=st.stats[x[0]]==null?'':st.stats[x[0]];inp.dataset.stat=x[0];wrap.appendChild(inp);}else{wrap.appendChild(el('div','thar-stat-value',st.stats[x[0]]==null?'—':st.stats[x[0]]));wrap.appendChild(el('div','thar-stat-unit','КГ'));}grid.appendChild(wrap);});
    card.appendChild(grid); return card;
  }

  function renderStatsEditor(){
    var b=body(); clear(); setTitle(st.group.name); b.appendChild(renderHero('Группа создана. Теперь добавь свои силовые, чтобы попасть в рейтинг.'));
    var card=renderStatsCard(true,true); var save=el('button','thar-btn red wide','Сохранить силовые'); save.dataset.busy='stats';
    save.addEventListener('click',async function(){var inputs=card.querySelectorAll('input[data-stat]');var bodyStats={};for(var i=0;i<inputs.length;i++){var key=inputs[i].dataset.stat;var raw=String(inputs[i].value||'').trim();if(!raw){bodyStats[key]=null;continue;}var n=Number(raw);if(!Number.isFinite(n)||n<0||n>1000){inputs[i].style.borderColor='#ef233c';inputs[i].focus();return;}bodyStats[key]=Math.round(n*100)/100;}busy('stats',true);var r=await API.request('PUT','/api/arena/stats',bodyStats);busy('stats',false);if(!alive())return;if(!r.ok){clear();body().appendChild(networkFail(r,renderStatsEditor));return;}st.stats=r.body.stats;openGroup(st.group.id,false);});
    card.appendChild(save); b.appendChild(card);
    var note=el('div','thar-card');note.appendChild(el('div','thar-sub','Что дальше'));note.appendChild(el('div','thar-note','После сохранения откроется общий рейтинг группы. Ты сможешь менять результаты в любой момент.'));b.appendChild(note);
  }

  async function openGroup(gid,forceStats){
    st.view='group'; st.group=null; setTitle('Загрузка…'); var b=body(); clear(); b.appendChild(skeletons(4));
    try{
  ...
  function renderGroupShell(editStats){
    var b=body(); clear(); setTitle(st.group.name); b.appendChild(renderHero('Твоя группа · общий силовой рейтинг'));
    var info=el('div','thar-card'); var row=el('div','thar-row'); row.appendChild(el('div','thar-name',st.group.name)); row.appendChild(el('div','thar-role',ROLE_LABEL[st.role]||'участник')); info.appendChild(row); info.appendChild(el('div','thar-note','Участников: '+(st.group.member_count||'—')));
    var inv=el('button','thar-btn red wide','🔗 Пригласить по ссылке'); inv.dataset.busy='invite'; inv.addEventListener('click',showInvite); info.appendChild(inv);
  ...
  async function showInvite(){busy('invite',true);var r=await API.request('POST','/api/groups/'+st.group.id+'/invite',{});busy('invite',false);if(!alive())return;var b=body();clear();setTitle(st.group.name);if(!r.ok){b.appendChild(networkFail(r,renderGroupShell));return;}var link=inviteLink(r.body.code);var card=el('div','thar-card');card.appendChild(renderHero('Одна ссылка — и участник сразу откроет Train Hard и попадёт в твою группу.'));
    var box=el('div','thar-link-box');box.appendChild(el('div','thar-sub','Ссылка-приглашение'));var a=el('div','thar-link',link);a.title='Нажми, чтобы скопировать';a.addEventListener('click',function(){try{navigator.clipboard&&navigator.clipboard.writeText(link);a.textContent='Ссылка скопирована ✓';setTimeout(function(){a.textContent=link;},1800);}catch(e){}});box.appendChild(a);card.appendChild(box);
    var cp=el('button','thar-btn red wide','Скопировать ссылку');cp.style.marginTop='10px';cp.addEventListener('click',function(){try{if(navigator.clipboard){navigator.clipboard.writeText(link);cp.textContent='Скопировано ✓';setTimeout(function(){cp.textContent='Скопировать ссылку';},1800);}}catch(e){}});card.appendChild(cp);
    var share=el('button','thar-btn ghost wide','Поделиться приглашением');share.style.marginTop='8px';share.addEventListener('click',function(){try{var tg=window.Telegram&&window.Telegram.WebApp;if(tg&&tg.openTelegramLink){tg.openTelegramLink(link);return;}if(navigator.share){navigator.share({title:'Train Hard — приглашение в группу',text:'Вступай в мою группу Train Hard',url:link});return;}window.open(link,'_blank');}catch(e){}});card.appendChild(share);
    card.appendChild(el('div','thar-note',cfg.inviteTtlNote||'Ссылка одноразовая и действует ограниченное время.'));
    var back=el('button','thar-btn ghost wide','← К группе');back.style.marginTop='10px';back.addEventListener('click',function(){openGroup(st.group.id,false);});card.appendChild(back);b.appendChild(card);
  }

  function boot(){ensureDom();if(!API.available())return;var sp=API.startParam();if(sp&&/^[A-Za-z0-9_-]{8,64}$/.test(sp)&&!wasInviteProcessed(sp)){setTimeout(openModal,150);}}
